...
for (let i = 1; i < Object.keys(marvelBigObject).length; i++) {
  moviesArray.push({
    'id': i,
    'title': marvelBigObject[i].title,
    'year': marvelBigObject[i].year,
    'director': marvelBigObject[i].director,
    'screenwriter': marvelBigObject[i].screenwriter,
    'producer': marvelBigObject[i].producer,
    'stage': marvelBigObject[i].stage
  })
}
...
for (i = 0; i < moviesArray.length; i++) {
  phaseCount[moviesArray[i].stage]++;
}
...
for (i = 0; i < moviesArray.length; i++) {
  moviesByPhase[moviesArray[i].stage].push(moviesArray[i].title);
}
...
for (let i = 0; i < moviesArray.length; i++) {
  if (moviesArray[i].year > ((new Date()).getYear() + 1900) ||
    moviesArray[i].year == 'TBA') {
    upcomingMoviesCount++;
  }
}

console.log('ЗАДАЧА 5');
console.log(upcomingMoviesCount);
console.log('\n\n\n\n');
...
for (i = 0; i < moviesArray.length; i++) {
  if (!moviesByYear[moviesArray[i].year]) {
    moviesByYear[moviesArray[i].year] = 1;
  } else {
    moviesByYear[moviesArray[i].year]++;
  }
}
...
for (i = 0; i < moviesArray.length; i++) {
  if (String(moviesArray[i].title).toLowerCase().includes('человек')) {
    moviesWithMan.push(moviesArray[i]);
  }
}
...
for (i = 0; i < moviesArray.length; i++) {
  if (moviesArray[i].year > 2018) {
    moviesAfter2018.push(moviesArray[i]);
  }
}
...
for (i = 0; i < moviesArray.length; i++) {
  if (moviesArray[i].producer.includes('Кевин Файги')) {
    moviesWithKevinFeige.push(moviesArray[i]);
  }
}
...
let earliestMovie = null;

for (i = 0; i < moviesArray.length; i++) {
  if (moviesArray[i].year < earliestYear) {
    earliestYear = moviesArray[i].year;
    earliestMovie = moviesArray[i];
  }
}
